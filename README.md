<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,20,24&height=200&section=header&text=Reyhan&fontSize=60&fontColor=ffffff&fontAlignY=38&desc=Engineering%20%7C%20AI%2FML%20%7C%20Competitive%20Programming&descAlignY=58&descSize=18&animation=fadeIn" width="100%"/>
</p>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3000&pause=800&color=6D5FA6&center=true&vCenter=true&multiline=false&width=700&lines=AI%2FML+Engineer+in+the+Making;Supervised+%26+Unsupervised+Learning;Computer+Vision+%7C+Deep+Learning;Building+at+the+Intersection+of+Logic+%26+Art;Sophomore+%40+Thapar+Institute+of+Engineering" alt="Typing SVG" />
  </a>
</p>

<br/>

<p align="center">
  <img src="https://img.shields.io/badge/B.Tech-Computer%20Science%20%26%20Engineering-6D5FA6?style=flat-square&logo=graduation-cap&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/Thapar%20Institute-Patiala%2C%20India-5A4E8A?style=flat-square&logo=university&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/JEE%20Advanced-Qualified-4A3F75?style=flat-square&logo=target&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/JEE%20Mains-97.38%25ile-3D3566?style=flat-square&logo=star&logoColor=white"/>
</p>

<br/>

<p align="center">
  <a href="https://leetcode.com/u/CodeByREYHAN/" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-5A4E8A?style=for-the-badge&logo=leetcode&logoColor=FFA116"/>
  </a>
  &nbsp;
  <a href="https://codeforces.com/profile/Erebus_Reyhan" target="_blank">
    <img src="https://img.shields.io/badge/Codeforces-4A3F75?style=for-the-badge&logo=codeforces&logoColor=white"/>
  </a>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/erebus-reyhan314159/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-3D3566?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="mailto:rh11reyhan@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-2E2850?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://github.com/code-rh11Reyhan" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-221E3C?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=code-rh11Reyhan&style=flat-square&color=6D5FA6&label=Profile+Views"/>
  &nbsp;
  <img src="https://img.shields.io/github/followers/code-rh11Reyhan?style=flat-square&color=5A4E8A&label=Followers&logo=github"/>
  &nbsp;
  <img src="https://img.shields.io/github/stars/code-rh11Reyhan?style=flat-square&color=4A3F75&label=Stars&logo=github"/>
</p>

<br/>

---

## ◈ About

<p align="center">
<table>
<tr>
<td width="55%" valign="top">

```
alias    →  Erebus
role     →  AI/ML Learner · CS Sophomore
year     →  Sophomore (B.Tech CSE, 2027)
org      →  Thapar Institute of Engineering
          & Technology, Patiala
focus    →  Machine Learning · Computer Vision
trait    →  Vision-driven · Curiosity-led
          · Relentless when given a target
```

</td>
<td width="45%" valign="top">

```python
class Erebus:
    institute = "TIET Patiala"
    jee_main  = "97.38 percentile"
    jee_adv   = "Qualified"

    tracks = [
      "AI / ML Engineering",
      "Competitive Programming",
      "Open-Source"
    ]

    hobbies = [
      "Poetry", "Theatre", "Guitar",
      "Chess", "Badminton"
    ]

    mantra = "Let the result speak."
```

</td>
</tr>
</table>
</p>

I started writing code in **9th grade** — building calculators and text editors in `tkinter`, crafting pixel games in `pygame` long before most of my peers had opened a terminal. A brief discontinuity in classes 11–12 (board grind + JEE), then back at full velocity in **Semester 1 of college**.

Today I am learning and building at the intersection of **machine learning** and **algorithmic thinking** — working through supervised and unsupervised learning, getting my hands dirty with computer vision, and grinding problem sets in parallel. My edge isn't purely technical: I bring a poet's ear for abstraction, a theatre actor's discipline under pressure, and a quiet refusal to stop.

> *I am in love with technology the way some people are in love with music — obsessively, structurally, and with full awareness that mastery is a direction, not a destination.*

<br/>

---

## ◈ Tech Stack

<p align="center">

**Core Languages**

<img src="https://skillicons.dev/icons?i=python,cpp,c,js,html,css&theme=dark&perline=6"/>

</p>

<p align="center">

**AI / ML & Data Science**

<img src="https://skillicons.dev/icons?i=tensorflow,sklearn,opencv,numpy&theme=dark&perline=4"/>

</p>

<p align="center">

**Tools & Platforms**

<img src="https://skillicons.dev/icons?i=git,github,vscode,bash&theme=dark&perline=4"/>

</p>

<p align="center">

| Domain | Stack | Depth |
|:---|:---|:---:|
| Machine Learning | scikit-learn · NumPy · Pandas | ████████░░ |
| Computer Vision | OpenCV · MediaPipe | ██████░░░░ |
| Deep Learning | TensorFlow · Keras | █████░░░░░ |
| Data Viz | Matplotlib · Plotly | ███████░░░ |
| App Prototyping | Streamlit | ██████░░░░ |
| Game Dev | Pygame | ███████░░░ |
| Competitive Prog. | C++ · STL · CSES | █████░░░░░ |

</p>

<br/>

---

## ◈ AI / ML Learning & Expertise

<p align="center">

| Domain | Proficiency | Details |
|:---|:---:|:---|
| **Supervised Learning** | ●●●●○ | Regression, SVM, Decision Trees, Ensemble methods via scikit-learn |
| **Unsupervised Learning** | ●●●○○ | K-Means, DBSCAN, PCA — applied in hackathon pipelines |
| **Computer Vision** | ●●●○○ | MediaPipe landmark pipelines, LAB colorspace analysis, YOLOv9c fine-tuning |
| **Deep Learning** | ●●○○○ | Learning — TensorFlow/Keras foundations, building toward PyTorch |
| **Data Engineering** | ●●●○○ | NumPy · Pandas · feature engineering for CV and tabular data |

</p>

<br/>

---

## ◈ Featured Projects

<details>
<summary><b>WiSense — WiFi-Based RF Object Detection</b> &nbsp; <img src="https://img.shields.io/badge/Eclipse%20Hackathon%206.0-ML%20%26%20Deployment%20Lead-6D5FA6?style=flat-square"/></summary>

<br/>

A camera-free object detection system that senses presence through WiFi Channel State Information (CSI) signal distortion — no hardware beyond a standard WiFi adapter required. Built in 24 hours at Eclipse Hackathon 6.0. Live demo deployed on Streamlit.

| Attribute | Detail |
|:---|:---|
| **Stack** | Python 3.11 · NumPy · SciPy · OpenCV · scikit-learn · Streamlit |
| **Pipeline** | CSI signal → Preprocessing → 2D spatial heatmap → Canny edge detection → SVM classifier |
| **Model** | SVM trained on 29 CSI features; validated against real Widar3.0 hardware-captured data |
| **Dataset** | Widar3.0 — IEEE Dataport (Zheng et al., IEEE TPAMI 2021) |
| **Research** | RF-Pose (Zhao et al., CVPR 2018) · WiGest (Abdelnasser et al., INFOCOM 2015) |
| **Role** | ML Lead & Deployment Lead |
| **Live Demo** | [wisense.streamlit.app](https://wisense-b4dfjicsgagn3v5coabpaq.streamlit.app/) |
| **Repository** | [`code-rh11Reyhan/WiSense`](https://github.com/code-rh11Reyhan/WiSense) |

**Engineering decisions:** Chose CSI-based sensing as a non-invasive detection method grounded in peer-reviewed RF research. Used Canny edge detection to extract object boundary features from reconstructed heatmaps before classification. SVM selected for its effectiveness on the 29-feature CSI vector with limited training samples. Streamlit used for rapid, zero-infrastructure deployment of the live demo within the hackathon window.

<br/>
</details>

<br/>

---

## ◈ Experience

<p align="center">

```
Currently: Sophomore — Year 1 Complete, Year 2 In Progress
```

</p>

<p align="center">

| Period | Role | Organisation |
|:---:|:---|:---|
| 2025 | **CORE MEMBER** | Creative Computing Society — CCS |
| 2025 | **ML Lead & Deployment Lead** | Eclipse Hackathon 6.0 — WiSense |
| 2025 | **ML Lead & Deployment Lead** | Makeathon 8.0 — Pirate Prufen |
| Ongoing | **Open-Source Contributor** | Independent — GitHub |

</p>

Experience is building. I am one year into a four-year programme. The next chapters are being written now.

<br/>

---

## ◈ Skills & Achievements

<p align="center">
  <img src="https://img.shields.io/badge/Class%20X-96%25-6D5FA6?style=flat-square&logo=award&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/Class%20XII-94.4%25-5A4E8A?style=flat-square&logo=award&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/JEE%20Mains-97.38%25ile-4A3F75?style=flat-square&logo=target&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/JEE%20Advanced-Qualified-3D3566?style=flat-square&logo=star&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/Student%20of%20the%20Year-School%20Award-2E2850?style=flat-square&logo=medal&logoColor=white"/>
</p>

<p align="center">

| Credential | Detail |
|:---|:---|
| **TIET Patiala** | B.Tech Computer Science & Engineering — Sophomore |
| **JEE Advanced** | Qualified — national-level engineering entrance |
| **JEE Mains** | 97.3799127 percentile |
| **Grade XII** | 94.4% — CBSE |
| **Grade X** | 96% — CBSE |
| **Student of the Year** | School academic award |

</p>

Beyond scores: I approach every domain with the same systematic curiosity — whether that's machine learning, theatrical performance, or competitive programming. A hovering mind is not a liability when paired with a clear target.

<br/>

---

## ◈ Certifications

<p align="center">
  <img src="https://img.shields.io/badge/Deep%20Learning%20Specialization-Coursera-6D5FA6?style=flat-square&logo=coursera&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/More%20Uploading%20Soon-In%20Progress-4A3F75?style=flat-square&logo=clock&logoColor=white"/>
</p>

<p align="center">

| Certificate | Issuer | Status |
|:---|:---|:---:|
| **Deep Learning Specialization** | deeplearning.ai / Coursera | ✓ Completed |
| Additional credentials | Various — uploading in progress | ⏳ Pending |

</p>

<br/>

---

## ◈ Coding Profiles

<p align="center">
  <a href="https://leetcode.com/u/CodeByREYHAN/" target="_blank">
    <img src="https://img.shields.io/badge/LeetCode-CodeByREYHAN-6D5FA6?style=for-the-badge&logo=leetcode&logoColor=FFA116&labelColor=1a1a2e"/>
  </a>
  &nbsp;&nbsp;
  <a href="https://codeforces.com/profile/Erebus_Reyhan" target="_blank">
    <img src="https://img.shields.io/badge/Codeforces-Erebus__Reyhan-5A4E8A?style=for-the-badge&logo=codeforces&logoColor=white&labelColor=1a1a2e"/>
  </a>
</p>

<p align="center">
  <a href="https://github.com/code-rh11Reyhan" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-code--rh11Reyhan-3D3566?style=for-the-badge&logo=github&logoColor=white&labelColor=1a1a2e"/>
  </a>
</p>

<p align="center">

| Platform | Handle | Focus |
|:---|:---|:---|
| **LeetCode** | [CodeByREYHAN](https://leetcode.com/u/CodeByREYHAN/) | Data Structures & Algorithms |
| **Codeforces** | [Erebus_Reyhan](https://codeforces.com/profile/Erebus_Reyhan) | Competitive Programming |
| **GitHub** | [code-rh11Reyhan](https://github.com/code-rh11Reyhan) | Open-Source · Project Portfolio |

</p>

<br/>

---

## ◈ GitHub Analytics

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=code-rh11Reyhan&show_icons=true&theme=midnight-purple&include_all_commits=true&count_private=true&hide_border=true&bg_color=0d0d1a&title_color=6D5FA6&icon_color=6D5FA6&text_color=b0a3d4&ring_color=6D5FA6"/>
  &nbsp;
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=code-rh11Reyhan&layout=compact&langs_count=8&theme=midnight-purple&hide_border=true&bg_color=0d0d1a&title_color=6D5FA6&text_color=b0a3d4"/>
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=code-rh11Reyhan&theme=midnight-purple&hide_border=true&background=0d0d1a&stroke=6D5FA6&ring=6D5FA6&fire=8B6FBF&currStreakNum=b0a3d4&sideNums=b0a3d4&currStreakLabel=6D5FA6&sideLabels=6D5FA6&dates=6b7280" width="60%"/>
</p>

<br/>

---

## ◈ GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=code-rh11Reyhan&theme=darkhub&no-frame=true&no-bg=true&column=7&margin-w=6&rank=SECRET,SSS,SS,S,AAA,AA,A,B" width="100%"/>
</p>

<br/>

---

## ◈ Contribution Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=code-rh11Reyhan&bg_color=0d0d1a&color=6D5FA6&line=6D5FA6&point=8B6FBF&area=true&area_color=2E2850&hide_border=true&custom_title=Contribution%20Graph" width="95%"/>
</p>

<br/>

### Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/code-rh11Reyhan/code-rh11Reyhan/output/github-contribution-grid-snake-dark.svg" alt="contribution snake" width="100%"/>
</p>

<br/>

---

## ◈ Current Focus

```yaml
# erebus :: active_state :: 2025

learning:
  machine_learning:
    current:      "Deepening supervised & unsupervised learning"
    framework:    "scikit-learn · NumPy · Pandas"
    next:         "Neural networks — TensorFlow foundations"

  computer_vision:
    current:      "OpenCV · MediaPipe pipelines"
    building_on:  "WiSense & SkinMax hackathon experience"

  competitive_programming:
    resource:     "CSES Problem Set"
    current:      "Data structures · greedy · basic DP"

  language:
    target:       "Spanish"
    status:       "Consistently deprioritised — acknowledged"

building:
  open_source:    "Exploring first meaningful OSS contribution"

exploring:
  deep_learning:  "Building intuition before scaling complexity"
  streamlit:      "Rapid ML prototyping and demos"
```

<br/>

---

## ◈ Connect

<p align="center">
  <a href="https://github.com/code-rh11Reyhan" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-code--rh11Reyhan-6D5FA6?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://www.linkedin.com/in/erebus-reyhan314159/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-erebus--reyhan314159-5A4E8A?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="mailto:rh11reyhan@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-Reach%20Out-4A3F75?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

<br/>

---

<p align="center">
  <i>"I am the universe's way of looking at itself — and I intend to make that count."</i>
</p>

<br/>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,20,24&height=120&section=footer&animation=fadeIn" width="100%"/>
</p>
